...
async function bootstrap() {
  ...
  try {
    logger.log('Starting BackupOps Control Plane API NestJS Application...');
    ...
    logger.error(`FATAL: Control Plane API failed to bootstrap: ${error.message}`, error.stack);
    process.exit(1);
  }
}

bootstrap().catch((err) => {
  console.error('FATAL: Control Plane API bootstrap promise rejected:', err);
  process.exit(1);
});